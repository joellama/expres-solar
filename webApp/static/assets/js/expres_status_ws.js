...
function build_status_table(){
    /* Create the header */
    /* Create a table for the status messages */
    modules = expres_modules; /* stored in global variable */
    
    $('#expres_status').html('')
        .append(
            $('<table id="expres_status_list" class="status"></table>')
                .append('<thead></thead>')
                .append('<tbody></tbody>')
        ); /* empty out anything that is there and start a table*/
    
    /* get references to the header and body of the table */
    sTableHead = $('#expres_status_list thead');
    sTableBody = $('#expres_status_list tbody');
    ...
    $(sTableHead).append(
        $('<tr></tr>')
            ...
            .append('<th>Module</th>')
            .append('<th>Status</th>')
            .append('<th>Sub-Modules</th>')
        );
    
    /* add a special row for expres and solar connection */
    $(sTableBody).append(moduleRow({"name": "solar", "pretty": "Solar Telescope"}));
    $(sTableBody).append(moduleRow({"name": "expres", "pretty": "EXPRES MGR"}));
    
    /* Now add each of the sub-modules */
    for (var modIdx in modules) {
        module = modules[modIdx];
        row = moduleRow(module);
        $(sTableBody).append(row);
    }
    
    /* Now that the table exists, fill out the status */
    getAllStatus();
}
...
function moduleRow( module ){
    ...
    btnStr = '<a href="javascript:getStatus(' + module.name + ');void(0);" class="button small icon now-ui-icons media-2_sound-wave"></a> ';
